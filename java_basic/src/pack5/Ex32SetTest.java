...
// 많은 수의 데이터를 그 사용 목적에 적합한 자료 구조로 묶어 하나로 그룹화 한 객체를 말함
// 자바에서는 이러한 컬렉션을 위한 인터페이스와 클래스들이 있다
// 컬렉션의 종류는 ArrayList, LinkdeList, Vector, Stack, HashSet, TreeSet, HashMap,TreeMap 등이 있으며, 크게 아래와 같은 자료구조 유형에 따라 구분
// List : 이름과 같이 목록처럼 데이터를 순서에 따라 관리. 중복 허용
// Set : 중복이 허용되지 않는 데이터를 관리
// Map : 데이터를 Key와 Value로 짝을 이루어 관리하며, Key값은 중복을 허용하지 않음
...
public class Ex32SetTest {	// 중복 불허, 순서 없음
	...
	public static void main(String[] args) {
		...
		// Iterator : 반복자는 객체 지향적 프로그래밍에서 배열이나 그와 유사한 자료구조의 내부 요소를 순회하는 객체다
		// 출력 방법1
